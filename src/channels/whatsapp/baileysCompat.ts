 ...
 *
 * Consumers (jid.ts) must go through this module instead of reaching into
 * raw Baileys objects directly. This ensures that when the Baileys API changes
 * (e.g. v7 LID primacy, structured lidMapping), only this file needs updating.
 *
 * Design rules:
...
export function lookupLidMapping(
  ...
  if (
    ...
    const value = (
      mapping as { getPNForLID: (lid: string) => unknown }
    ).getPNForLID(lidKey);
  ...
  if (
    ...
    return stringOrUndefined(
      (mapping as { get: (key: string) => unknown }).get(lidKey),
    );
  }
