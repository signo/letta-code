...
describe("resolveInboundChatId", () => {
  ...
  test("LID remoteJid → resolves to PN via signalRepository.lidMapping", () => {
    const lidMapping = new Map<string, string>([
      ["9876543210@lid", "584149145006@s.whatsapp.net"],
    ]);
    const ctx = makeCtx({
      sock: { signalRepository: { lidMapping } },
    });
    const result = resolveInboundChatId(ctx, "9876543210:5@lid", false, {
      key: {},
    });
    expect(result).toBe("584149145006@s.whatsapp.net");
  });
